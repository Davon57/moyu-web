<template>
  <div class="single-rank-card">
    <div class="card-header">
      <span class="card-icon">{{ platformConfig.icon }}</span>
      <h3 class="card-title">{{ platformConfig.name }}</h3>
...
const platformConfig = computed(() => {
  const configs = {
    douyin: { name: '抖音热搜', icon: '🎵', api: '/v2/douyin' },
    weibo: { name: '微博热搜', icon: '🔴', api: '/v2/weibo' },
    bili: { name: 'B站热搜', icon: '📺', api: '/v2/bili' }
  }
  return configs[props.platform] || configs.douyin
})
...
const loadData = async () => {
  loading.value = true
  try {
    const response = await fetch(`${API_BASE}${platformConfig.value.api}?encoding=json`)
