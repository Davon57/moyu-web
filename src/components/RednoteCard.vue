<template>
  <div class="single-rank-card rednote-card">
    <div class="card-header">
      <span class="card-icon">📕</span>
      <h3 class="card-title">小红书热点</h3>
    ...
    <div class="rank-list">
      ...
      <ul v-else class="rank-items">
        <li v-for="item in list" :key="item.rank" class="rank-item">
          ...
          <span class="rank-hot">{{ item.wordType }}</span>
        </li>
      </ul>
    </div>
  </div>
...
const loadData = async () => {
  loading.value = true
  try {
    const response = await fetch(`${API_BASE}/v2/rednote?encoding=json`)
